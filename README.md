...
- 선택 캐릭터의 공개 프로필 전신 이미지를 왼쪽, 장착 유물 6개의 이미지와 옵션을 오른쪽에 한 번에 표시
- 가져온 유물의 주옵션 이름과 실제 계수, 부옵션, 점수를 카드에서 함께 표시
- 가져온 유물을 카드에서 입력칸에 적용하거나 최대 300개씩 나눠 전체 점수 계산
...
5. 왼쪽에서 현재 캐릭터의 공개 프로필 전신 이미지를 보고, 오른쪽에서 장착 유물 최대 6개의 이미지와 옵션을 한 번에 확인합니다.
6. 각 유물 카드의 `주옵션 계수`에서 `치명타 피해 64.8%`, `속도 25.03`처럼 실제 수치를 확인합니다.
7. 유물 한 개를 직접 입력 화면으로 옮기려면 해당 카드의 `입력 적용`을 누릅니다.
8. 평가 프로필을 고르고 `전체 점수 계산`을 누르면 각 카드 아래에 등급과 점수가 표시됩니다. 캐릭터 탭을 바꾸면 해당 캐릭터의 장착 화면으로 즉시 전환됩니다.
9. UID 방식은 공개 프로필에 전시된 최대 8명 캐릭터의 장착 유물만 조회합니다.
...
캐릭터와 유물 이미지는 MiHoMo 응답의 `portrait`, `icon` 경로를 StarRailRes 원본 주소에 연결해 표시합니다. MiHoMo 응답에는 별도 착용 의상 ID가 없으므로 캐릭터 쪽은 공개 프로필 전신 이미지를 사용하며, 이미지 표시에는 인터넷 연결이 필요합니다.
...
4. 오른쪽 유물 카드에서 `입력 적용`을 누르거나 `전체 점수 계산`을 누릅니다.
5. 카드에서 각 유물의 주옵션 계수와 부옵션을 함께 확인합니다.
