...
- 가져온 유물을 입력칸에 적용하거나 최대 300개씩 나눠 전체 점수 계산·정렬하고 주옵션·부옵션 함께 표시
...
3. `UID 불러오기`를 누릅니다.
4. `캐릭터별 보기`에서 확인할 캐릭터 탭을 누릅니다. 탭 아래에는 해당 캐릭터가 장착한 유물만 표시됩니다.
5. 유물 한 개를 골라 `선택 유물 적용`을 누르거나 평가 프로필을 선택하고 `전체 점수 계산`을 누릅니다.
6. 점수 결과에는 `주옵션 공격력% · 부옵션 치명타 확률 6.48% · …`처럼 주옵션과 부옵션이 함께 표시됩니다.
7. 전체 계산 후에도 캐릭터 탭을 바꾸면 해당 캐릭터의 점수 결과만 볼 수 있습니다.
8. UID 방식은 공개 프로필에 전시된 최대 8명 캐릭터의 장착 유물만 조회합니다.
...
4. 선택한 캐릭터의 유물을 적용하거나 `전체 점수 계산`을 누릅니다.
5. 점수 결과에서 각 유물의 주옵션과 부옵션을 함께 확인합니다.
