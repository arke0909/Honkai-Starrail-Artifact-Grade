...
- 캐릭터 일러스트 아래에 평균 점수, 등급 분포, 세트 구성, 핵심 주옵션 수, 교체 우선 유물과 핵심 부옵션 기여 합계 표시
...
6. 일러스트 아래 `RELIC OVERVIEW`에서 유물 평균 점수, 등급 분포, 세트 구성과 핵심 주옵션 개수를 확인합니다. `교체 우선`은 현재 장착 유물 중 최저점과 주옵션·비유효 부옵션 사유를 알려줍니다.
7. 각 유물 카드의 `주옵션 계수`에서 `치명타 피해 64.8%`, `속도 25.03` 같은 실제 수치와 `핵심·유효·보조·비유효`, 가중치, 점수 기여도를 확인합니다.
8. 별도 버튼을 누르지 않아도 주옵션과 부옵션을 합친 선택 캐릭터 전용 점수와 등급이 자동 표시됩니다.
9. `캐릭터 유효 옵션 전체`에서 12개 부옵션의 가중치와 `핵심·유효·보조·비유효` 판정을 확인합니다.
10. 각 유물 카드에서는 실제 부옵션마다 같은 판정, 가중치와 점수 기여도를 확인합니다. 캐릭터 탭을 바꾸면 새 캐릭터 기준으로 즉시 다시 계산합니다.
11. UID 방식은 공개 프로필에 전시된 최대 8명 캐릭터의 장착 유물만 조회합니다.
