...
- 실제 API: 카스토리스 `max=9.24`를 적용한 +15 머리 샘플에서 고정 HP 주옵션은 50점, HP%·치확·치피 최고 품질 롤은 각각 6.5점, 공격력%는 0점으로 계산되어 총 69.5점(B)이 나왔다. 캐릭터 자동 평가는 B 60, A 70, S 80, SS 90, SSS 97점 이상 구간을 사용한다.
