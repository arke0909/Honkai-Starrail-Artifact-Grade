...
열한 번째는 정보량을 우선하다가 작아진 글자였다. 캐릭터 옵션명은 8px, 유물 부옵션 설명은 6.5~7.5px까지 내려가 실제 사용 화면에서 빠르게 읽기 어려웠다. 캐릭터 옵션과 유물의 주·부옵션, 점수, 종합 요약 텍스트를 10~15px 중심으로 키우고, 반대로 첫 화면의 큰 제목은 최대 59px에서 54px로 조금 줄였다. 430px 이하에서는 캐릭터 옵션을 3열에서 2열로 바꿔 긴 옵션명도 말줄임 없이 보이도록 했다.
